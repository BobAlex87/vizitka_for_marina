...
async def get_start(message: Message, bot: Bot):
    ...
    await message.answer_photo(
        photo,
        caption=f'Привет {message.from_user.first_name}👋 рад тебя видеть!'
                         f'\nМеня зовут Марина и я рада с вами познакомиться!'
                         f'Тут вы познакомитесь c НОВЫМ проектом, как можно заработать на минимальных вложениях '
                         f'пассивный доход до 2000 $ в месяц'
                         f'И что ничего не надо делать?  -  Хотела бы я сказать ДА!!! '
                         f'ведь мы все именно этого и ищем - как так зарабатывать и ничего не делать'
                         f'И тут этот вариант возможен а если хочешь большего, '
                         f'то все же поработать  и что-то поделать придется'
                         f'\n<b>Начнем!</b>',
